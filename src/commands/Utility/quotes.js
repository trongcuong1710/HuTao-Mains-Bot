...
class QuotesCommand extends Command {
  ...
  async exec(message, args) {
    const quotes = await this.client.db.huTaoQuotes.find();
    ...
    const quoteIsRegistered = await this.client.db.huTaoQuotes.findOne({
    ...
    const quoteOfName = await this.client.db.huTaoQuotes.find({
      quoteName: args.quoteName,
    });
